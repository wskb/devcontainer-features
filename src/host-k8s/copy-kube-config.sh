...
LOCAL_K8S_HOST="%localk8shost%"

# Copies localhost's ~/.kube/config file into the container and swaps out localhost
# for the provided host whenever a new shell starts to keep them in sync.
if [ -d "/usr/local/share/kube-localhost" ]; then
    mkdir -p $HOME/.kube
    sudo cp -r /usr/local/share/kube-localhost/* $HOME/.kube
    sudo chown -R $(id -u) $HOME/.kube
    sed -i -e "s/localhost/$LOCAL_K8S_HOST/g" $HOME/.kube/config
    sed -i -e "s/127.0.0.1/$LOCAL_K8S_HOST/g" $HOME/.kube/config
